...
CREATE DATABASE db_infertility_system;
USE db_infertility_system;
...
CREATE TABLE treatment_step_lab_test (
                                         id INT PRIMARY KEY AUTO_INCREMENT,
                                         ...
                                         FOREIGN KEY (treatment_step_id) REFERENCES treatment_step(id) ON DELETE CASCADE
);

CREATE OR REPLACE VIEW doctor_dashboard_overview AS
SELECT
    d.id AS doctor_id,
...
CREATE OR REPLACE VIEW manager_work_statistics_today_view AS
SELECT
    1 AS id,
    (
        SELECT COUNT(*)
    ...
    ) AS completed_patients_today
;

CREATE OR REPLACE VIEW manager_revenue_overview AS
SELECT
    (
    ...
    (
        ...
        FROM payment_transaction pt
        WHERE pt.status = 'SUCCESS'
          AND pt.created_at >= CONVERT_TZ(DATE_FORMAT(NOW(), '%Y-%m-01'), 'Asia/Ho_Chi_Minh', '+00:00')
          AND pt.created_at <  CONVERT_TZ(DATE_FORMAT(DATE_ADD(NOW(), INTERVAL 1 MONTH), '%Y-%m-01'), 'Asia/Ho_Chi_Minh', '+00:00')
